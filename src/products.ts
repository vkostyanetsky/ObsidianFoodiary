    ...
    private static async addProduct(plugin: Foodiary, products: Array<Product>, file: TFile) {
        ...
        if (productProperties != undefined) {
            ...
            const product = {
				...
				titles: titles,
				value: {
					calories: this.propertyValue(
						productProperties[plugin.settings.propertyCalories]
					),
					protein: this.propertyValue(
						productProperties[plugin.settings.propertyProtein]
					),
					fat: this.propertyValue(
						productProperties[plugin.settings.propertyFat]
					),
					carbs: this.propertyValue(
						productProperties[plugin.settings.propertyCarbs]
					),
					fiber: this.propertyValue(
						productProperties[plugin.settings.propertyFiber]
					),
					cholesterol: this.propertyValue(
						productProperties[plugin.settings.propertyCholesterol]
					),
					sodium: this.propertyValue(
						productProperties[plugin.settings.propertySodium]
					),
				},
			};
